...
Adding a variable damper, {% latex %} B_{semi} {% endlatex %}, with matrix N to \eqref{eq:ss-passive} results in the semi-active model \eqref{eq:ss-semi}.
...
![MLP]({{site.url}}/img/mlp.png)

Figure 1: MLP cross entropy loss for training and test sets with four settings of momentum velocity, {% latex %} \alpha {% endlatex %}, up to 500 training epochs, fixed learning rate of 0.01, and 250 hidden units.
...
The Reynold's number, Re, was read from Figure~\ref{fig:moody}, with friction factor, {% latex %} f {% endlatex %}, and relative roughness, {% latex %} k/d {% endlatex %}, and given the assumptions stated in Section~\ref{sec:background}.

![Moody]({{site.url}}/img/moody.png)

Moody diagram available in public domain \protect\url{https://grantingram.wordpress.com/2009/04/22/moody-diagram/}. Axis not quite legible at default zoom, but there are only so many examples of figures that you use to calculate something, and in public domain.
...
The counting\_task}, shown in Listing~\ref{counting_task}, increments a counter everytime LCD\_count semaphore is posted in __event\_task__, up to COUNT\_MAX value, and then rolls over to 0.

Altera __counting\_task__ with priority 9.
